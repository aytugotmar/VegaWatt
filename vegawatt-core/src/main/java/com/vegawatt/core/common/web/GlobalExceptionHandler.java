...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    // Overrides ResponseEntityExceptionHandler's default handling so a @Valid request-body
    // failure returns the same RFC7807 shape (title/detail via problem()) as every other 400 in
    // this API, instead of Spring's own differently-shaped default body.
    @Override
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
        ...
        return ResponseEntity.status(status)
                .body(problem(HttpStatus.BAD_REQUEST, "Validation failed", detail));
    }
